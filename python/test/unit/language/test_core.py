...
def test_abs_fp8(in_dtype, device):
    if is_hip():
        ...
        if in_dtype == tl.float8e4nv and cc < (8, 9) and cc != (7, 5):
...
def test_histogram(M, N, device):
    ...
    if is_cuda() and not is_interpreter() and torch.cuda.get_device_capability() != (7, 5):
        # ptxas picks this shared-memory increment from sm80 on.

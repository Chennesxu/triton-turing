...
LogicalResult Prefetcher::initialize() {
  ...
  for (Operation &op : *loop) {
    ...
    // Upstream groups these five under nvidia_gpu::TMAOpInterface, which this
    // revision predates; the concrete ops are the same set.
    if (isa<triton::nvidia_gpu::AsyncTMACopyGlobalToLocalOp,
            ...
            triton::nvidia_gpu::AsyncTMAScatterOp>(op)) {

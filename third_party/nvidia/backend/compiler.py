...
class CUDABackend(BaseBackend):
    ...
    def make_ttgir(mod, metadata, opt, capability):
        ...
        passes.common.add_canonicalizer(pm)
        passes.ttir.add_loop_aware_cse(pm)
        # On sm75 the pass only fires on multi-slot sync-copy rings; the knob
        # is there to compare against the unsplit loop. Other targets keep the
        # behaviour they had.
        if capability != 75 or opt.sm75_prefetch:
